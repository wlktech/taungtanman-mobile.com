        ...
        <table class="mt-3 border table table-striped">
          ...
          <tbody>
           ...
           <tr>
            ...
            <td><small class="badge text-bg-{{ $order->order->status == 'pending' ? 'primary' : '' }}{{ $order->order->status == 'delivering' ? 'info' : '' }}{{ $order->order->status == 'completed' ? 'success' : '' }}">{{ $order->order->status }}</small></td>
